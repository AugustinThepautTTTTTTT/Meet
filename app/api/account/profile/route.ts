...
export async function PUT(request: Request) {
  ...
  try {
    ...
    if (!text(p.name) || !text(p.specialty)) return NextResponse.json({ error: "Name and professional title are required." }, { status: 400 });
    const sql = getLawyerDb();
    const baseSlug = text(p.name).toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "lawyer";
    const slug = `${baseSlug}-${accountId.slice(0, 6)}`;
    const tags = list(p.tags);
    const reasons = list(p.reasons);
    const initials = text(p.name).split(/\s+/).map(part => part[0]).join("").slice(0, 2).toUpperCase();
    const [profile] = await sql`
      INSERT INTO lawyers (account_id,slug,initials,name,specialty,practice,location,languages,price,availability,accent,reasons,bio,experience,credentials,tags,keywords,published)
