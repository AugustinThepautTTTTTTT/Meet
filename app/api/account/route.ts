...
export async function GET() {
  ...
  const [[account], [profile]] = await Promise.all([
    sql`SELECT id,name,email,created_at FROM lawyer_accounts WHERE id=${accountId}`,
    sql`SELECT * FROM lawyers WHERE account_id=${accountId}`,
  ]);
  ...
  const inquiries = profile
    ? await sql`SELECT id, client_name, client_email, brief, meeting_time, meeting_start,
        meeting_uid, invite_sent_at, status, lawyer_note, payment_status,
        payment_amount_cents, payment_currency, created_at, updated_at
        FROM inquiries WHERE lawyer_id=${profile.id} ORDER BY created_at DESC`
    : [];
