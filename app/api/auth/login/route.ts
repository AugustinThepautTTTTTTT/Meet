...
export async function POST(request: Request) {
  try {
    ...
    const [account] = await sql`SELECT id,password_hash FROM lawyer_accounts WHERE email=${String(email || "").trim().toLowerCase()}`;
    if (!account || !(await compare(String(password || ""), account.password_hash))) {
